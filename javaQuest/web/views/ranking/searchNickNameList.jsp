<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.jqt.ranking.model.vo.Ranking"%>
<% 
	Ranking list = (Ranking)request.getAttribute("list");
		...
		<div class="tableArea">
			...
			<table align="center">
				...
				<tr>
					...
					<td><%= list.getUserMsg() %></td>
					<%if(type == "Level"){ %>
					<td><%= list.getUserLevel() %></td>
					<% }else if(type == "Timeattack"){ %>
					<td><%= list.getTimeAttack() %></td>
					<% }else{ %>
					<td><%= list.getWinRate() %></td>
					<% } %>
				</tr>
				<% } %>
			</table>
		</div>
		<form action="<%= request.getContextPath() %>/searchNickName.ro" method="post">
